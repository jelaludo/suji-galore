...
export const systems = [
  ...
  system({id:'kaktovik',name:'Kaktovik',ja:'イヌピアック数字',base:20,origin:'Unicode',structure:'Cumulative positional',
    ...
    layout(n){const ds=digits(n,20),parts=[],groups=[];ds.forEach((d,i)=>{const x=(ds.length-1-i)*112+6;const f=Math.floor(d/5),o=d%5;
      ...
      const five=[[15,32,82,15],[15,32,82,49],[15,66,82,49]];for(let j=0;j<f;j++){const g=five[j];parts.push(line(`${i}:f${j+1}`,x+g[0],g[1],x+g[2],g[3],0));}
      const one=[[15,32,40,104],[40,104,60,47],[60,47,76,104],[76,104,92,47]];for(let j=0;j<o;j++){const g=one[j];parts.push(line(`${i}:o${j+1}`,x+g[0],g[1],x+g[2],g[3],1));}
