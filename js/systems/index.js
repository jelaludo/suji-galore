...
export const systems = [
  ...
  system({id:'kaktovik',name:'Kaktovik',ja:'イヌピアック数字',base:20,origin:'Unicode',structure:'Cumulative positional',
    ...
    layout(n){const ds=digits(n,20),parts=[],groups=[];ds.forEach((d,i)=>{const x=(ds.length-1-i)*112+6;const f=Math.floor(d/5),o=d%5;
      ...
      const five=[[24,61,88,44],[88,44,24,27],[24,27,88,10]];for(let j=0;j<f;j++){const g=five[j];parts.push(line(`${i}:f${j+1}`,x+g[0],g[1],x+g[2],g[3],0));}
      const one=[[24,61,42,126],[42,126,62,70],[62,70,78,126],[78,126,94,70]];for(let j=0;j<o;j++){const g=one[j];parts.push(line(`${i}:o${j+1}`,x+g[0],g[1],x+g[2],g[3],1));}
      groups.push(`${d} × ${20**i} · ${f} fives + ${o}`);});return layout(parts,ds.length*112,132,groups);},
