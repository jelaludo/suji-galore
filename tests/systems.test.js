...
export function runTests(){
  ...
  for(const [n,fiveId,fivePoint] of [[6,'0:f1','start'],[11,'0:f2','end'],[16,'0:f3','start']]){const fp=coords(partFor(n,fiveId)),op=coords(partFor(n,'0:o1')),joint=fivePoint==='start'?fp.slice(0,2):fp.slice(2,4);assert(joint.join(',')===op.slice(0,2).join(','),`Kaktovik ${n} joins its five and one components`);}
