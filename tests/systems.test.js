...
export function runTests(){
  ...
  const k19=kaktovik.layout(19).parts,toY=part=>[...part.d.matchAll(/-?\d+(?:\.\d+)?/g)].map(m=>Number(m[0])).filter((_,i)=>i%2);const fiveBottom=Math.max(...k19.filter(p=>p.id.includes(':f')).flatMap(toY)),oneTop=Math.min(...k19.filter(p=>p.id.includes(':o')).flatMap(toY));assert(fiveBottom<oneTop,'Kaktovik five and one bands do not overlap');
